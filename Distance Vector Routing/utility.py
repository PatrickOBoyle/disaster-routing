...
def alert_neighbours(file_path, connections, router_port):
                    ...
                    if item_data != "NULL" and item_data != '':
                        ...
                        if our_data != "NULL":
                            if our_data == '':
                                ...
                                data_changed = True
                                times_data_changed += 1
                                
                                connection_table[row][router_index] = int(item_data) + COST_TO_ROUTER
                            # replace larger distance with shorter one only
                            
                            elif int(our_data) > (int(item_data) + 1):
                                # make a bool true for changed file
                                data_changed = True
                                times_data_changed += 1
    ...
    # If True, data has been modified and should be ran again
    if(times_data_changed > 0):
        return True
    else:
        return False
    
def alert_all_connections(router_nodes):
    alerts_ran = 0
    ...
    for router_node in router_nodes:
        ...
        data_updated = alert_neighbours(file_path, connections, router_node[0])
        
        if data_updated:
            alerts_ran += 1
    if alerts_ran > 0:
        alert_all_connections(router_nodes)
